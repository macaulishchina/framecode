package com.sinoyd.frame.views.swipelistview;
...
import com.sinoyd.frame.views.swipelistview.SwipeItemLayout.Mode;
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if(mCaptureItem!=null){
                    ...
                    if( touchMode== Mode.TAP ){
                        ...
                        if((deltaX < -mTouchSlop && xDiff> yDiff) || (deltaX > mTouchSlop && mCaptureItem.isOpen() && xDiff> yDiff)){
                            //从右往左滑动，拦截滑动事件，进行CaptureItem滑动
                            //从左往右滑动，若CaptureItem已打开，拦截滑动事件；若CaptureItem未打开，不拦截滑动事件，交给父控件处理
                            mDragHandleByThis = true;
                            mCaptureItem.setTouchMode(Mode.DRAG);
                            final ViewParent parent = getParent();
                            parent.requestDisallowInterceptTouchEvent(true);
